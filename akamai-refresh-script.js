...
function URLToRefresh () {
  ...
  var getResultSheet = SpreadsheetApp.getActiveSpreadsheet();
  ...
  var URLSList = new Array();
    ...
    if (URLForPayload.length > 0) {
      if (URLSList[i] != null) { URLForPayload = URLForPayload + ',\n"' + URLSList[i] + '"'; }
    } else {
  ...
  lastRow = lastRow + 1;
 // Browser.msgBox(URLForPayload.length);
  try {
    if (URLForPayload.length > 0) {
