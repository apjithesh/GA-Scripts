 ...
 *     parameters.
 */
function displayResults_(results, config, row, columnOutput,m) {

  // row and columnOutput is to see where to display and m is to see if need to show header and also if need to clear sheet
  ...
  //log_('Row value before core exec .' + row);
  //log_('Row count value before core exec .' + row.count);

  // Only output rows if they exist.
  if (results.getRows()) {
    var type = config[TYPE] || CORE_TYPE;  // If no type, default to core type.
    if (type == CORE_TYPE) {
      outputCoreRows_(results, activeSheet, row, columnOutput);
    } else if (type == MCF_TYPE) {
      outputMcfRows_(results, activeSheet, row);
    }
  }

  //log_('Row value after core exec .' + row);
  //log_('Row count value after core exec .' + row.count);
 ...
 */
function outputHeaders_(results, activeSheet, row, columnOutput) {
  var headerRange = activeSheet.getRange(row.count, columnOutput.count, 1, results.getColumnHeaders().length);
  var headerNames = [];
 ...
 */
function outputCoreRows_(results, activeSheet, row, columnOutput) {
  ...
  activeSheet.getRange(row.count, columnOutput.count,
                       results.getRows().length,
                       results.getColumnHeaders().length).setValues(rows);

  log_('Rows value . ' + rows.length);
